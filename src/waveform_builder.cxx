...
//*********************************************************************************
// Constructor KCU event (per FPGA & trigger sample)
//*********************************************************************************
kcu_event::kcu_event( uint32_t fpga, 
                      uint32_t num_asics, 
                      uint32_t samples, 
...
//*********************************************************************************
// check if the entire waveform is build
//*********************************************************************************
...
//*********************************************************************************
// check if the entire waveform is in correct order
//*********************************************************************************
bool kcu_event::is_ordered() {
    bool in_order = true;
    for (int i = 1; i < found; i++) {
      if (event_counter[i] != (event_counter[i - 1] + 1) % 64) {      // why am I using modulo 64? 
        in_order = false;
      }
    }
    return in_order;
...
//*********************************************************************************
// waveform_builder code starts here
//*********************************************************************************


//*********************************************************************************
// Constructor waveform_builder per FPGA (KCU)
//*********************************************************************************
waveform_builder::waveform_builder( uint32_t fpga_id, 
                                    uint32_t num_asics, 
...
//*********************************************************************************
// waveform builder for data version < v0.13
//*********************************************************************************
...
bool waveform_builder::build_v013(std::list<sample*> *samples) {
    ...
    for (auto sample_itr = samples->begin(); sample_itr != samples->end(); ) {
      ...
      if (in_progress->size() > 50){
        int delcount = 0;
        for (delcount = 0; delcount < 25 && in_progress->size() != 0; delcount++){ 
      ...
      for (auto event_itr = in_progress->begin(); event_itr != in_progress->end(); event_itr++) {
        ...
        if (sample->trigger_counter_Int == event->trigger_counter_Int && sample->trigger_counter_Ext == event->trigger_counter_Ext ) {
          ...
          //____________________________________________________________________
          // Check if kcu_event is now complete
          //____________________________________________________________________
          if (event->is_complete()) {
            ...
            // increase correct counters and move among lists
          ...
          //-------------------------------------------------------------
          // jump out here (already found event loop)
          //-------------------------------------------------------------
          break;
        }
      }
...
//*********************************************************************************
// unwrap counters for < v0.13 alignment
//*********************************************************************************
...
//*********************************************************************************
// get number of aborted waveforms
//*********************************************************************************
...
//*********************************************************************************
// Get number of correctly ordered waveforms
//*********************************************************************************
...
//*********************************************************************************
// Print info for Waveform in progress to debug log
//*********************************************************************************
...
//*********************************************************************************
// remove the first nEvt from list
//*********************************************************************************
void waveform_builder::drop_first( int nEvt){
    int delcount  = 0;
    for (delcount = 0; delcount < nEvt && complete->size() != 0; delcount++){ 
      auto event      = *complete->begin();
...
//*********************************************************************************
// remove event with matching trigCountInt & trigCountExt from list
//*********************************************************************************
...
//*********************************************************************************
// increase skipped event counter for event in list
//*********************************************************************************
